...
class DataProcessor:
    ...
    def process_reads(
        ...
        """
        self.logger.debug(
            f"process_reads called with label='{label}', contig={contig}, pos={target_position}"
        )
        ...
        if self.logger.isEnabledFor(logging.DEBUG):
            ...
            self.logger.debug(
                f"Alignment orientation: {forward_count} forward, {reverse_count} reverse"
            )
        ...
        if len(alignments) != len_fetched:
            self.logger.warning(
                f"Signal extraction incomplete: found signals for {len(alignments)} out of {len_fetched} reads"
            )
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    f"Lost {len_fetched - len(alignments)} reads during signal extraction"
                )
        else:
            self.logger.debug(
                f"Successfully extracted signals for all {len(alignments)} reads"
            )
    ...
    def _extract_alignments(
        ...
        self.logger.debug(
            f"_extract_alignments: BAM={bam_path.name}, {contig}:{target_position}"
        )

        # Check cache
        if bam_path not in self._alignment_cache:
            self.logger.debug(f"Creating new AlignmentExtractor for {bam_path.name}")
            self._alignment_cache[bam_path] = AlignmentExtractor(
                bam_path, logger=self.logger
            )
        else:
        ...
        try:
            ...
            self.logger.error(
                f"Alignment extraction failed: {type(e).__name__}: {str(e)}"
            )
            raise

    def _extract_signals(
        self, pod5_path: Path, alignments: List[ReadAlignment]
    ) -> List[ReadAlignment]:
        """Extract signals from POD5 file."""
        self.logger.debug(
            f"_extract_signals: POD5={pod5_path.name}, n_alignments={len(alignments)}"
        )
        ...
        try:
            ...
            alignments_with_signals = self._signal_cache[pod5_path].extract_signals(
                alignments
            )

            self.logger.debug(
                f"Signal extraction complete: {len(alignments_with_signals)} reads have signals"
            )

            # Log any reads that failed signal extraction
            if len(alignments_with_signals) < len(
                alignments
    ...
    def _log_alignment_statistics(self, alignments: List[ReadAlignment]):
        ...
        if coverage_by_pos:
            self.logger.debug(
                f"  - Average coverage: {np.mean(list(coverage_by_pos.values())):.1f}"
            )
        if signal_lengths:
            self.logger.debug(
                f"  - Signal lengths: min={min(signal_lengths)}, "
